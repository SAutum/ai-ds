...
def finish(
    ...
    for node in ast.walk(tree):
        ...
        if isinstance(node, ast.alias):
            assert (
                node.name not in disallowed_identifiers
            ), f'"{node.name}" is not allowed for this exercise.'
    ...
    for testfile in sorted(Path(".").glob("test_*.py")):
        ...
        if zlib.crc32(data) != 0:
            print_red(
                f"WARNING: It seems that you have modified {testfile}. This is not allowed."
            )

    # Check if parameters are passed to psycopg
    bad_connect = get_bad_psycopg_connect(code)
    assert (
        not bad_connect
    ), f"It looks like you are passing parameters ({bad_connect}) to psycopg.connect. This might break our tests, your solution will not work and get 0 points. Instead, use environment variables like PGUSER to configure your connection."

    print(
        f"Peak memory usage {peak*1e-6:.3f} MB, execution time {elapsed_time:.3f} seconds"
    )
    print("Tests passed :)")


def get_bad_psycopg_connect(code: str) -> Optional[str]:
    allowed_args = [
        "autocommit",
        "row_factory",
        "cursor_factory",
    ...
    for match in re.findall(r"psycopg\.connect\(([^)]+)\)", code, flags=re.DOTALL):
...
def check_file_size(filename: str, min_size: int, max_size: int) -> None:
    ...
    assert (
        size <= max_size
    ), f"File {filename} is larger than expected ({size} bytes). Expected no more than {max_size} bytes. Note that too long answers/too large files may give fewer or no points."

    print(
        f"File {filename} exists. Content will be checked manually after the deadline."
    )
...
def test_and_zip(
    assignment: str = "rdda-11", max_size: int = 4 * 10**6
) -> None:
    filenames = """
        data
        exercise_1_a.py
        ...
        test_exercise_2_a.py
        test_exercise_2_b.py
        test_exercise_3_a.py
    ...
    for library, library_version in read_versions(library_versions).items():
        if library not in installed_versions:
            ...
            incorrect_versions_list.append(
                f"{library} {installed_versions[library]} instead of {library_version}"
            )
